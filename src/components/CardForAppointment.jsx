  ...
  const { therapistsId,name, email, date, time, userEmail} = props;
    ...
    const handleBookAppointment = async () => {
      try {
        ...
        const response = await axios.post(`${baseUrl}api/book-appointment`, requestBody);
  ...
  return (
    <div className="w-[22rem] border-1 border-gray-400 p-4 shadow-lg bg-gray-100">
      ...
      <div className=" bg-blue-700 rounded-md  p-4 mt-2 cursor-pointer">
        <button className="text-white font-bold" onClick={handleBookAppointment}>Book Appointment</button>
      </div>
    </div>
  );
};
